...
The audit currently fails by design while `src/main.ts`, `src/styles.css`, and `tests/browser-smoke.ts` remain above the 500-line threshold. Keep that failure visible until those files are split or explicitly documented as exceptions.
